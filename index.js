...
for(let i = 0; i < addToBagButtons.length; i++) {
    ...
    addToBagButton.addEventListener('click', function (event) {
        ...
        // Check wheather input has valid value
        var input = document.getElementById('input-size-'+ (i + 1));
        var size = input.value.toLowerCase();
        var isValidSize = size == 's' || size == 'm' || size == 'l' || size == 'xl' || size == 'xs';
        if (isValidSize) {
            // show the Goto bag button on same card
            addToBagButton.classList.add('hide');

            var gotoBag = document.getElementById('goto-bag-'+ (i + 1))
            gotoBag.classList.remove('hide');
            gotoBag.setAttribute('href', 'file:///C:/Users/parka/Desktop/myntra/checkout.html');

            // Increase the bag count
            ...
            // alert('Added to bag.');
            // Change the title
            var title = document.querySelector('#validationModal #validationModalLabel');
            title.innerText = 'Hurray!!';
            // $('#validationModal')
            // Modal body change
            var modalBody = document.querySelector('#validationModal #validationModalBody');
            modalBody.innerText = 'Item added to bag successfully.';
            $('#validationModal').modal();
        } else {
            // alert('Added to bag.');
            // Change the title
            var title = $('#validationModal #validationModalLabel');
            title.html('Validation Error');
            // $('#validationModal')
            // Modal body change
            var modalBody = $('#validationModal #validationModalBody');
            modalBody.text('Please enter a valid size.');
            $('#validationModal').modal();
